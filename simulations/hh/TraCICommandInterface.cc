...
//double TraCICommandInterface::Vehicle::getVehicleSpeed()
double TraCICommandInterface::getVehicleSpeed()
{
    //return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_SPEED, RESPONSE_GET_VEHICLE_VARIABLE);
    return 0.0;
}

//double TraCICommandInterface::Vehicle::getVehicleAngle()
double TraCICommandInterface::getVehicleAngle()
{
    //return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, nodeId, VAR_ANGLE, RESPONSE_GET_VEHICLE_VARIABLE);
    return 0.0;
}
...
{
    return genericGetStringList(CMD_GET_PERSON_VARIABLE, "", ID_LIST, RESPONSE_GET_PERSON_VARIABLE);
}
Coord TraCICommandInterface::getPersonPosition(std::string personId)
{
    return genericGetCoord(CMD_GET_PERSON_VARIABLE, personId, VAR_POSITION, RESPONSE_GET_PERSON_VARIABLE);
}
...
std::string TraCICommandInterface::Person::getTypeId() {
    return traci->genericGetString(CMD_GET_PERSON_VARIABLE, personId, VAR_TYPE, RESPONSE_GET_PERSON_VARIABLE);
}

void TraCICommandInterface::Person::setSpeed(double speed) {
    uint8_t variableId = VAR_SPEED;
    uint8_t variableType = TYPE_DOUBLE;
    TraCIBuffer buf = traci->connection.query(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << variableId << personId << variableType << speed);
    ASSERT(buf.eof());
}

double TraCICommandInterface::Person::getSpeed() {
    return traci->genericGetDouble(CMD_GET_PERSON_VARIABLE, personId, VAR_SPEED, RESPONSE_GET_PERSON_VARIABLE);
}

uint8_t TraCICommandInterface::Person::getCurrentStage() {
    uint8_t variableId = VAR_STAGE;
    uint8_t variableType = TYPE_INTEGER;
    uint32_t nextStageIndex = 0;    // nextStageIndex = 0: retrieve value for the current stage
    TraCIBuffer buf = traci->connection.query(CMD_GET_PERSON_VARIABLE, TraCIBuffer() << variableId << personId << variableType << nextStageIndex);

    uint8_t cmdLength_resp; buf >> cmdLength_resp;
    uint8_t commandId_resp; buf >> commandId_resp; ASSERT(commandId_resp == RESPONSE_GET_PERSON_VARIABLE);
    uint8_t variableId_resp; buf >> variableId_resp; ASSERT(variableId_resp == variableId);
    std::string personId_resp; buf >> personId_resp; ASSERT(personId_resp == personId);
    uint8_t respType; buf >> respType; ASSERT(respType == variableType);
    uint32_t stage; buf >> stage;
    ASSERT(buf.eof());
